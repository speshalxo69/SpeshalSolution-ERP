    ...
    for (let index = 0; index < docs.length; index += 400) {
        ...
        chunk.forEach((docSnap) => {
            batch.update(docSnap.ref, {
                ownerUid, ownerEmail,
                transferredAt: timestamp, transferredBy: actorUid, updatedAt: timestamp,
            });
        });
        await batch.commit();
    }
...
async function handleDeleteUser(request, env) {
    ...
    let adminContext;
    try {
        adminContext = await requireAdmin(request, env);
    ...
    try {
        ...
        if (mode === 'inspect') {
            return json(200, {
                ok: true, uid, email: targetIdentity.email,
                role: targetIdentity.role || 'client', productsCount, categoriesCount,
        ...
        if (productsCount > 0 || categoriesCount > 0) {
            if (!transferToUid) return json(400, { error: 'This user still owns data. Transfer target is required before deletion.' });
        ...
        return json(200, {
            ok: true, uid, email: targetIdentity.email, deleted: true,
...
async function handleRepairUser(request, env) {
    ...
    if (!ALLOWED_ROLES.has(requestedRole)) return json(400, { error: 'Invalid role.' });
    ...
    try {
        ...
        const docData = {
            ...
            repairedAt: timestamp, repairedBy: decoded.uid, updatedAt: timestamp,
        };
        if (!userSnap.exists) {
        ...
        return json(200, {
            ok: true, uid: authUser.uid,
            email: authUser.email || email, role, existed: userSnap.exists,
        });
    } catch (error) {
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/api/delete-user-account') {
            return handleDeleteUser(request, env);
        }
        if (url.pathname === '/api/repair-user-by-email') {
            return handleRepairUser(request, env);
        }
